(function(define) { 'use strict';
	...
	function(require, exports, module, window) {
		...
		function SocketClient(config) {	
			this.config = config;	
			this.config.cacheDir = config.cacheDir || "./logs/"+this.config.host+":"+this.config.port+"Cache/";
			this.initCache();
		...
		(function() {
			...
			this.initCache = function() {
				...
				this.cacheMax = 10;
				latte_fs.mkdirSync(this.config.cacheDir);
				self.cacheFiles = latte_fs.getTimeSort(self.config.cacheDir) || [];
